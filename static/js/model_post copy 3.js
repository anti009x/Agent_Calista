        ...
        function read() {
          return reader.read().then(({done, value}) => {
            if (done) {
              ...
              if (matches.length > 0) {
                // Remove raw duplicate link texts from the displayed content
                ...
                // Use a Set to ensure each link appears only once
                const uniqueLinks = new Set(matches.map(match => match[0]));
                textElements.forEach(el => {
                  uniqueLinks.forEach(link => {
                    // Only append the link if it isn't already present
