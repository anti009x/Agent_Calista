function getNewResponseContainer() {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const submitButton = chatInput?.nextElementSibling;
          ...
          function read() {
            return reader.read().then(({done, value}) => {
              if (done) {
                ...
                const linkRegex = /https:\/\/[^\s]+(?="|,|\s|$)/g; // Match URLs up to quote, comma, space or end
                const matches = accumulatedText.match(linkRegex);
                if (matches && matches.length > 0) {
                  const uniqueLinks = [...new Set(matches.map(link => link.replace(/["',]+$/, '')))]; // Remove duplicates and trailing quotes/commas
    ...
    if (sendEditButton) {
      sendEditButton.addEventListener('click', handleCombinedSubmit);
    }
